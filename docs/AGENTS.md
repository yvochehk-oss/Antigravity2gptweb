# 成都建工 V3.1 Agent 开发与分支同步规则 (AGENTS.md)

本文件规定了 AI Agent 在本项目中进行代码修改、Git 同步、版本提交和跨平台治理的核心原则与行为准则。

---

## 0. V3.1 分支架构速查表（Agent 必读，禁止弄错分支）
...
* **唯一代码基线**：`main` 是项目的唯一主体基线。`v3.0-windows` 和 `v3.0-macos` 是对同一 V3.1 代码基线的平台镜像引用。
...
❌ **禁止** `git push origin windows` 或 `git push origin macos`——这两个是 V2.0 遗留废弃分支，推送到此处不会触发 V3.1 的 CI 镜像流水线，修复将永远不会进入生产线！
...
4. **/safari-chatgpt-reasoner Skill 执行核心行为守则 (Local Agent Behavioral Protocol)**：
   ...
   - **⑥ 读到回复即刻全量回显与执行透明化 (Echo & Transparency)**：本地 Agent 每次从 GPT 网页端读取到回复或检验命令（`TEST:`）后，**必须第一时间在聊天窗口中完整输出所读到的 GPT 原文及待执行命令清单**；随后执行命令时，必须将每一步的实际终端输出与退出码透明呈现在界面中，严禁静默黑盒执行！
