...
from ..models import (
    Budget, Contract, Fulfillment, Invoice, CashFlow, RealCost,
    Progress, Project, TaxRule,
)
...
def build_context(db: Session, pid: int, scope: str) -> dict[str, Any]:
    ...
    base: dict[str, Any] = {
        "project": {
            ...
            "city": p.city, "contract_total": float(p.contract_total),
    ...
    if cat:
        ...
        base["contracts"] = _serialize(contracts, [
            "contract_no", "buyer_code", "seller_code", "category",
            "amount", "internal_trade", "note",
        ])
        base["fulfillment"] = _serialize(fulfills, [
            "counterparty_code", "kind", "category", "quantity",
            "amount", "evidence_complete", "note",
        ])
        base["invoices"] = _serialize(invoices, [
            "invoice_no", "period", "entity_code", "direction",
            "counterparty_code", "category", "net", "vat", "rate",
            "deductible", "note",
        ])
        base["real_costs"] = _serialize(costs, [
            "entity_code", "counterparty_code", "category", "subcategory",
            "period", "amount", "external_cash", "note",
        ])
        base["four_stream_matching"] = [
            x for x in matching_rows(db, pid) if x["category"] == cat
        ]
        return base

    if scope in ("overview", "whole_project", "budget", "eac"):
        base["budget"] = _serialize(budgets, ["category", "amount"])
        base["progress"] = _serialize(progress, [
            "period", "output_value", "settlement",
            "recognized_revenue", "collection",
        ])
    ...
    if scope in ("cashflow", "whole_project"):
        base["cashflows"] = _serialize(cash, [
            "entity_code", "counterparty_code", "direction",
            "amount", "period", "note",
        ])
        base["four_stream_matching"] = matching_rows(db, pid)
    if scope in ("cost", "whole_project"):
        ...
        base["cost_tree"] = {
            k: {kk: float(vv) for kk, vv in v.items()} for k, v in tree.items()
        }
    if scope in ("tax", "whole_project"):
        ...
        base["tax_rule_review_status"] = _serialize(rules, [
            "code", "rate", "effective_from", "effective_to", "reviewed", "note",
        ])
    if scope in ("risk", "whole_project"):
        risks = scan_risks(db, pid)
        base["deterministic_risks"] = _serialize(risks, [
            "severity", "code", "message", "resolved",
        ])
    if scope == "eac":
        base["eac_note"] = (
            "EAC为系统确定性管理预测；模型只能解释和提出需要复核的假设，"
            "不得覆盖系统计算。"
        )

    # V0.2: 整项目截断改为"显式摘要"，不静默丢数
    if scope == "whole_project":
        for key in ("contracts", "fulfillment", "invoices", "cashflows", "real_costs"):
            if key in base and len(base[key]) > 80:
                truncated = base[key][:80]
                base[key] = truncated
                base[f"{key}_truncated_summary"] = (
                    f"已截断为前 80 条；本字段原本 {key} 数据应进一步分段或聚合后再发模型。"
                )
    return base