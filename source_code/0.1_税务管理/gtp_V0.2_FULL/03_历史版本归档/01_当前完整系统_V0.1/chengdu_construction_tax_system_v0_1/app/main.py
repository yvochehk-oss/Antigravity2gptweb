...
def health_check_home(r:Request,project_id:int|None=None):
    ...
    db.close()
    return t.TemplateResponse("health_check.html",{"request":r,"projects":projects,"endpoints":endpoints,
...
def health_check_detail(r:Request,batch_id:int):
    ...
    db.close()
    return t.TemplateResponse("health_check_detail.html",{"request":r,"batch":batch,"project":project,"jobs":jobs,
...
def ai_prompts(r:Request):
    ...
    return t.TemplateResponse("ai_prompts.html",{"request":r,"rows":rows,"scopes":SCOPES})
...
def task_center(r:Request):
    ...
    endpoints=db.execute(select(AIModelEndpoint).where(AIModelEndpoint.enabled==True).order_by(AIModelEndpoint.id)).scalars().all()
    db.close(); return t.TemplateResponse("tasks.html",{"request":r,"rows":rows,"projects":projects,"endpoints":endpoints,"scopes":SCOPES})
