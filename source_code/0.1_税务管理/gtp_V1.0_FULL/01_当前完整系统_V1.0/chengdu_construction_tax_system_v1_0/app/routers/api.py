...
def api_delete_project_data(pid: int, payload: DeleteProjectDataRequest, user=Depends(require_role("admin"))):
    ...
    try:
        ...
        rag_preserve_tables = {
            "projects", "documents", "chunks", "ingest_jobs", "canonical_facts",
            "canonical_fact_outbox", "accounting_report_snapshots",
        }
        tables_with_project_id = [
            str(name)
            for name in db.execute(text("SELECT DISTINCT table_name FROM information_schema.columns WHERE table_schema='public' AND column_name='project_id'")).scalars().all()
        ]
