...
class S32Monitor:
    ...
    def __init__(self) -> None:
        ...
        self.dml = re.compile(r'^\s*(?:INSERT\s+INTO|UPDATE|DELETE\s+FROM)\s+"?([A-Za-z0-9_]+)"?', re.I)
    ...
    def before_cursor_execute(self, conn, cursor, statement, parameters, context, executemany) -> None:
        ...
        if self.watch_legacy_reads and sql.lstrip().upper().startswith("SELECT"):
            ...
            if tables & self.LEGACY_READ_TABLES:
                self.flags["canonical_web_legacy_business_read_attempted"] = True
                raise AssertionError(f"Gate S32 canonical Web read touched legacy business tables: {sorted(tables & self.LEGACY_READ_TABLES)}")
...
def _counts(session: Session) -> dict[str, int]:
    ...
    result = {}
    for name in names:
        exists = session.execute(text("SELECT to_regclass(:name)"), {"name": f"public.{name}"}).scalar_one()
        result[name] = int(session.execute(text(f'SELECT count(*) FROM "{name}"')).scalar_one()) if exists else -1
    return result


def main() -> int:
    failures: list[str] = []
    evidence: dict[str, object] = {"gate": "S32", "alembic_head": EXPECTED_HEAD}
    ...
    try:
        ...
        try:
            ...
            req(failures, evidence, "production_route_guard_reused", all(getattr(route, k) == v for k, v in EXPECTED_ROUTE.items()), "Production route mismatch")
            ...
            with TestClient(app) as client:
                ...
                req(failures, evidence, "unauthenticated_v3_access_rejected", unauth.status_code == 401, "Unauthenticated V3 request was accepted")

                middleware.current_user_from_request = lambda request: SimpleNamespace(id=1, username="gate-s32", role="admin", is_active=True)
                status = client.get("/api/v3/system/status")
                req(failures, evidence, "system_status_ready", status.status_code == 200 and status.json().get("ready") is True and status.json().get("alembic_head") == EXPECTED_HEAD, "V3 system status not ready")
                ...
                four_expected = CanonicalFourFlow(session).read(project.id, eligible_fact_ids=set(finance_expected.get("eligible_fact_ids", [])))
                ...
                fact_state0 = [(r.id, r.validation_status, r.business_identity_key, r.version_no, r.supersedes_fact_id) for r in session.scalars(select(Fact).where(Fact.id.in_([contract.id, invoice.id, payment.id, unlinked.id, review.id]))).all()]
                alloc0 = [(r.id, r.fact_id, r.project_id, str(r.allocated_gross), r.status) for r in session.scalars(select(FactProjectAllocation).where(FactProjectAllocation.fact_id.in_([contract.id, invoice.id, payment.id, unlinked.id, review.id]))).all()]
                rel0 = [(r.id, r.source_fact_id, r.target_fact_id, r.relationship_type, r.reason) for r in session.scalars(select(FactRelationship).where(FactRelationship.source_fact_id.in_([invoice.id, payment.id]))).all()]
                ...
                rag_http = client.get(f"/api/v3/boss/projects/{project.id}/rag-context?scope=whole_project")
                quality_http = client.get(f"/api/v3/boss/projects/{project.id}/evidence-quality")
                snap_http = client.get(f"/api/v3/boss/projects/{project.id}/snapshot")
                monitor.watch_legacy_reads = False

                req(failures, evidence, "boss_finance_matches_task31", finance_http.status_code == 200 and finance_http.json() == jsonable_encoder(finance_expected), "Boss finance differs from Task31")
                req(failures, evidence, "boss_four_flow_matches_task31", four_http.status_code == 200 and four_http.json() == jsonable_encoder(four_expected), "Boss four-flow differs from Task31")
                req(failures, evidence, "boss_tax_matches_task31", tax_http.status_code == 200 and tax_http.json() == jsonable_encoder(tax_expected), "Boss tax differs from Task31/Task17")
                req(failures, evidence, "boss_rag_matches_task31", rag_http.status_code == 200 and rag_http.json().get("context") == jsonable_encoder(rag_expected) and rag_http.json().get("data_source") == "CANONICAL_FACTS", "Boss RAG differs from native canonical context")
                req(failures, evidence, "relationship_coverage_reported", quality_http.status_code == 200 and "relationship_coverage" in quality_http.json(), "Relationship coverage missing")
                no_inference = four_http.status_code == 200 and four_http.json().get("invoice_payment_amount_allocation", {}).get("supported") is False
                req(failures, evidence, "invoice_payment_amount_not_inferred", no_inference, "Payment relationship was converted into invoice paid amount")
                req(failures, evidence, "boss_snapshot_api_native_v3", snap_http.status_code == 200 and snap_http.json().get("data_source") == "CANONICAL_FACTS" and snap_http.json().get("finance") == finance_http.json() and snap_http.json().get("four_flow") == four_http.json(), "Boss snapshot is not a native V3 aggregate")

                fact_state1 = [(r.id, r.validation_status, r.business_identity_key, r.version_no, r.supersedes_fact_id) for r in session.scalars(select(Fact).where(Fact.id.in_([contract.id, invoice.id, payment.id, unlinked.id, review.id]))).all()]
                alloc1 = [(r.id, r.fact_id, r.project_id, str(r.allocated_gross), r.status) for r in session.scalars(select(FactProjectAllocation).where(FactProjectAllocation.fact_id.in_([contract.id, invoice.id, payment.id, unlinked.id, review.id]))).all()]
                rel1 = [(r.id, r.source_fact_id, r.target_fact_id, r.relationship_type, r.reason) for r in session.scalars(select(FactRelationship).where(FactRelationship.source_fact_id.in_([invoice.id, payment.id]))).all()]
                ...
                inv_res = client.post("/api/v3/idp/direct", json=inv_req.model_dump(mode="json"))
                req(failures, evidence, "invoice_e2e_completed", inv_res.status_code == 200 and inv_res.json().get("validation_status") == "VALID", f"Invoice HTTP E2E failed: {inv_res.text}")
                ...
                pay_res = client.post("/api/v3/idp/direct", json=pay_req.model_dump(mode="json"))
                req(failures, evidence, "payment_e2e_completed", pay_res.status_code == 200 and pay_res.json().get("validation_status") == "VALID" and pay_res.json().get("payment_evidence") is not None, f"Payment HTTP E2E failed: {pay_res.text}")
                ...
                con_res = client.post("/api/v3/idp/direct", json=con_req.model_dump(mode="json"))
                req(failures, evidence, "contract_review_e2e_completed", con_res.status_code == 200 and con_res.json().get("validation_status") == "NEEDS_REVIEW" and con_res.json().get("contract_roles") is not None, f"Contract HTTP E2E failed: {con_res.text}")
                req(failures, evidence, "idp_direct_api_reuses_task27", inv_res.status_code == pay_res.status_code == con_res.status_code == 200, "Task27 production API path failed")
        ...
        with Session(engine) as post:
            req(failures, evidence, "production_e2e_fixture_rolled_back", baseline == _counts(post), "Gate S32 leaked fixture/application writes")
    ...
    evidence["canonical_web_legacy_overlay_removed"] = not bool(evidence.get("canonical_web_legacy_business_read_attempted"))
    evidence["automatic_fact_supersession_present"] = False
    evidence["production_e2e_gate_passed"] = not failures
    for key in ("legacy_write_attempted", "production_seal_write_attempted", "cutover_state_write_attempted", "canonical_web_legacy_business_read_attempted"):
        if evidence.get(key):
            failures.append(f"Forbidden operation detected: {key}")
    if failures:
        evidence["production_e2e_gate_passed"] = False
    print(json.dumps({"gate":"S32", "status":"PASS" if not failures else "FAIL", "evidence":evidence, "failures":failures}, ensure_ascii=False, indent=2, sort_keys=True, default=str))
