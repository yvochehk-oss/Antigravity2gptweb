...
from app.services.canonical_project_summary import canonical_project_summary, resolve_project_transaction_price
...
def test_resolver_prefers_known_main_contract(monkeypatch):
    ...
    result = resolve_project_transaction_price(FakeDB(), 15)
...
def test_resolver_fails_closed_when_boundary_contracts_are_ambiguous(monkeypatch):
    ...
    with pytest.raises(ValueError, match="ambiguous canonical project transaction price"):
        resolve_project_transaction_price(FakeDB(), 15)
...
def test_api_project_source_has_no_legacy_project_summary_call():
    ...
    assert "canonical_project_summary" in api_block
    assert "project_summary(" not in api_block
