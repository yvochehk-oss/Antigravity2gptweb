...
> 历史迁移说明：本文中的旧主体编号只用于描述旧版数据库语义。当前运行版已统一使用 26 家实际单位的 canonical Entity 代码（A01-A11、B01-B10、C01-C02、D01-D03），外部交易方使用 ExternalParty；旧占位主体不得进入当前数据库。
...
业务角色 A/B/C/D 只保留在 `business_role` 字段中，不能作为 `entity_code`。所有交易、台账与页面提交必须使用完整的实际单位代码；A04 为 A03 的非独立法人分支机构。外部交易方必须使用真实名称/外部方代码，识别失败则保持 unresolved。

## 8. 不会破坏

- 业务结果（数字相同）
- 主体边界（canonical Entity 实际单位 / ExternalParty 真实交易方）
