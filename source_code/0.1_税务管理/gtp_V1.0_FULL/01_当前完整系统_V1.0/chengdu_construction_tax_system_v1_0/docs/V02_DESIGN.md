...
`constants.py` 唯一定义：
- `BUSINESS_ROLES = frozenset({"A", "B", "C", "D"})`（仅业务角色，不是法人代码）
- canonical Entity 代码集合（A01-A11、B01-B10、C01-C02、D01-D03）
- ExternalParty 真实交易方代码；无法唯一识别时使用 unresolved/unknown external party 状态
...
- 所有表单、接口和计算输入必须引用 canonical Entity 的完整实际单位代码，不得使用单字业务角色作为 `entity_code`。
- 26 家单位中 25 家为独立法人；A04 为非独立法人分支机构，主数据通过 `parent_entity_code=A03` 记录归属，税务计算按归集规则处理。
- 外部交易方不进入 Entity 表，使用 ExternalParty 的真实名称、外部方代码和可核验统一社会信用代码；识别不充分时保留 unresolved 状态并进入数据缺口。
