...
> 历史测试报告：原始 V0.2 结果中的主体示例属于旧版占位语义。当前验收以 canonical Entity 主数据和 ExternalParty 真实交易方为准；当前主体相关回归见 `tests/test_entity_master.py` 与 `tests/test_tax_canonical_entities.py`。
...
| 文件 | 用例 | 覆盖 |
|---|---|---|
| `test_calc.py` | 4 | `_note_category` 词边界 / 四流匹配证据 / 实际法人独立税务 / 风险阈值覆盖 |
...
- 外部设备交易履约证据缺失 → 状态"履约证据不完整" ✅

#### test_tax_ledger_independent
- 实际法人按 canonical Entity 代码独立生成台账 ✅
- 有销项业务的实际法人销项 VAT > 0 ✅
- CIT note 包含"未应用项" ✅
