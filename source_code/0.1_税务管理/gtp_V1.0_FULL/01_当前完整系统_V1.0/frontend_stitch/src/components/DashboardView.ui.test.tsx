...
const ledgerRecords = [
  {
    ...
    inputVat: 40,
    taxPrepayment: 5,
    vatPayableBeforePrepayment: 40,
    closingInputCredit: 8,
    vatPayableAfterPrepayment: 35,
...
describe('DashboardView dual-core executive summaries', () => {
  it('renders statutory and project scopes as separate KPI sections without calling contract-cost gap real profit', () => {
    ...
    expect(within(statutory).getByText('LEGAL_ENTITY_STATUTORY · 法人法定申报口径')).toBeInTheDocument();
    ...
    expect(within(project).getByText('PROJECT_BOUNDARY · 项目管理/测算口径')).toBeInTheDocument();
    expect(within(project).getByText('Tax API 项目数量')).toBeInTheDocument();
  ...
  it('does not hide one domain when the other domain is unavailable', () => {
    ...
    expect(within(health).getByText('法人 Statutory VAT')).toBeInTheDocument();
    expect(within(health).getByText('项目 Project Boundary')).toBeInTheDocument();
  });
});
