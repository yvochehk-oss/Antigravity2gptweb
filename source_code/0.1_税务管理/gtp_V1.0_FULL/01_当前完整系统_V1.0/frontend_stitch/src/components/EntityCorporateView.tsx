...
export function EntityCorporateView() {
  ...
  const activeEntity = useMemo(
    () => legalEntities.find(item => item.canonicalCode === entityCode),
    [legalEntities, entityCode],
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-3 rounded-xl border border-[#444653]/30 bg-[#171f33]/50 p-3">
          <label className="flex min-w-[220px] max-w-[320px] flex-col gap-1 text-[11px] text-[#8e909f]">
            ...
            <select
              aria-label="法人主体"
              value={entityCode}
              disabled={masterStatus !== 'READY' || legalEntities.length === 0}
            ...
            >
              ...
              {legalEntities.map(item => (
                <option key={item.partyId} value={item.canonicalCode}>
                  {item.canonicalCode} · {item.legalName}
                </option>
              ))}
            </select>
          </label>
      ...
      <section aria-label="经营 Projection" className="space-y-4 rounded-2xl border border-[#8b5cf6]/30 bg-[#8b5cf6]/5 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-[15px] font-bold text-[#ddd6fe]"><Building2 className="h-4 w-4" />经营 Projection</div>
          ...
          <span className="rounded-full border border-[#8b5cf6]/30 px-2.5 py-1 text-[10px] text-[#c4b5fd]">
            ...
            {activeEntityName && activeEntityName !== entityCode && <span className="ml-1 text-[#e9d5ff]">（{activeEntityName}）</span>}
          </span>
        </div>
      ...
      <section aria-label="正式 VAT" className="space-y-4 rounded-2xl border border-[#4cd7f6]/30 bg-[#03b5d3]/5 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-[15px] font-bold text-[#4cd7f6]"><ReceiptText className="h-4 w-4" />正式 VAT</div>
        ...
        {statutoryRecords.length > 1 && (
          <div role="status" className="flex items-center gap-2 rounded-lg border border-[#F59E0B]/30 bg-[#F59E0B]/5 px-3 py-2 text-[12px] text-[#ffd0a8]">
            <AlertTriangle className="h-4 w-4" />DEGRADED · 同一法人/期间返回多条正式 VAT 当前记录，未在前端合并，请复核后端 current ledger 约束。
          </div>
        )}
