...
def score_metadata_confidence(
    ...
    for field, value in metadata.items():
        ...
        raw = str(value).strip()
        conf = _score_field(field, raw, source, base_low, base_high)
        scores[field] = FieldConfidence(value=raw, confidence=conf)

    return scores


def _score_field(field: str, raw: str, source: str, base_low: float, base_high: float) -> float:
    ...
    if fn == "period":
        ...
        # Single character is more reliable (A/B/C/D pattern)
