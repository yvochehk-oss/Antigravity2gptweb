...
from app.services.metadata import is_canonical_entity_code as is_canonical_entity_code_meta
from app.services.metadata_confidence import is_canonical_entity_code as is_canonical_entity_code_conf
...
def test_metadata_canonical_code_accepts_canonical_and_rejects_aliases():
    ...
    for code in ("A01", "A11", "B01", "B10", "C01", "C02", "D01", "D03", "E01", "E02", "EA01", "EA02", "EB01", "EB02", "EB03", "EC01", "ED01"):
        assert is_canonical_entity_code_meta(code), f"Expected canonical in metadata: {code}"
        assert is_canonical_entity_code_conf(code), f"Expected canonical in metadata_confidence: {code}"
    ...
    for code in invalid_codes:
        assert not is_canonical_entity_code_meta(code), f"Expected non-canonical in metadata: {code}"
        assert not is_canonical_entity_code_conf(code), f"Expected non-canonical in metadata_confidence: {code}"
