...
class _FakeSession:
    ...
    def execute(self, statement, params=None):
        if self.error:
            raise self.error
        sql = str(statement)
        if "facts_snapshots" in sql:
            return _Result(self.historical)
...
def _complete_row(**changes):
    row = {
        ...
        "entity_mapping_reason": None,
        "entity_mapping_valid": True,
        "calculated_at": "2026-08-20T00:00:00+08:00",
        "facts_available": True,
        "contract_amount": 110.0,
        "recognized_revenue": 100.0,
        "real_project_cost": 60.0,
        "real_profit": 40.0,
...
def test_metric_and_response_serialization_is_json_safe():
    response = FactsResponse(
        ...
        metrics={"real_profit": MetricValue(40, "2.1", "CNY")},
    )
...
def test_missing_required_metric_is_degraded_without_partial_financial_output():
    response = FactsProvider(_FakeSession(current=_complete_row(eac_profit=None))).get_facts(
        "P-001", require_fresh=True
    )
...
def test_invalid_required_metric_is_degraded():
    response = FactsProvider(_FakeSession(current=_complete_row(real_profit="not-a-number"))).get_facts(
        "P-001", require_fresh=True
    )
    assert response.status == "DEGRADED" and response.metrics == {}
...
def test_source_completeness_flag_is_respected():
    response = FactsProvider(_FakeSession(current=_complete_row(facts_available=False))).get_facts(
        "P-001", require_fresh=True
    )
    assert response.status == "DEGRADED" and response.metrics == {}
...
def test_missing_entity_mapping_is_degraded_without_aggregate_metrics():
    response = FactsProvider(
        _FakeSession(
            current=_complete_row(
                entity_code=None,
    ...
    ).get_facts("P-001", require_fresh=True)
    assert response.status == "DEGRADED" and response.facts_available is False
    assert response.metrics == {}
    assert "entity mapping gap" in (response.reason or "")


def test_invalid_entity_mapping_including_business_role_is_degraded():
    response = FactsProvider(
        _FakeSession(
            current=_complete_row(
                entity_code="A",
                entity_mapping_status="INVALID",
                entity_mapping_valid=False,
                entity_mapping_reason="project.entity_code is not a canonical numbered entity code",
            )
        )
    ).get_facts("P-001", require_fresh=True)
    assert response.status == "DEGRADED" and response.metrics == {}
    assert "invalid entity_code" in (response.reason or "")


def test_valid_canonical_entity_mapping_remains_available():
    response = FactsProvider(
        _FakeSession(
            current=_complete_row(
                entity_code="A01",
    ...
    assert response.status == "AVAILABLE" and response.facts_available is True
    assert response.metrics["real_profit"].value == 40.0
    assert response.entity_code == "A01"
...
def test_historical_snapshot_without_entity_mapping_is_degraded():
    payload = {
        "facts_available": True,
        "metrics": {
            "recognized_revenue": {"value": 100},
            "real_profit": {"value": 40},
            "collection_rate": {"value": 0.2},
            "eac_profit": {"value": 35},
            "eac_margin": {"value": 0.31},
        },
    }
    row = {"facts_data": json.dumps(payload), "as_of": "2026-07-31T00:00:00+08:00", "facts_version": "f_history"}
    response = FactsProvider(_FakeSession(historical=row)).get_facts("P-001", as_of="2026-08-01T00:00:00+08:00")
