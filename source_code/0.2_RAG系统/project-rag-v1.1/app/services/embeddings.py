...
# Use the same dimension as BGE-M3 for consistency
...
def _hash_embedding(text: str) -> list[float]:
    ...
    Uses HASH_DIM (same as EMBEDDING_DIM = 1024) for consistency.
...
def embed_many(texts: list[str], raise_on_error: bool = False) -> list[list[float]]:
    ...
    # Production guard: hash fallback is disabled in production
    is_production = os.getenv("APP_ENV", "").lower() in ("production", "prod")
    if EMBEDDING_BACKEND == "hash_v1":
        if is_production:
            raise EmbeddingError(
                "Embedding backend 'hash_v1' is not permitted in production. "
                "Set APP_ENV != production or use EMBEDDING_BACKEND=bge_m3."
            )
        logger.debug("Using hash embedding fallback (dev only)")
        ...
        try:
            ...
            for i, v in enumerate(vectors):
                ...
                if len(vec_list) == EMBEDDING_DIM:
                    ...
                    if is_production:
                        raise EmbeddingError(
                            f"Vector dimension mismatch: got {len(vec_list)}, expected {EMBEDDING_DIM}. "
                            "Cannot use hash fallback in production."
                        )
                    valid_vectors.append(_hash_embedding(texts[i]))
            ...
            if is_production:
                raise EmbeddingError(
                    "BGE model unavailable and hash fallback is not permitted in production. "
                    "Either install FlagEmbedding or set APP_ENV != production."
                )
            ...
            if is_production:
                raise EmbeddingError(
                    f"Embedding generation failed and hash fallback is not permitted in production: {e}"
                )
    ...
    if is_production:
        raise EmbeddingError(
            f"Unknown embedding backend '{EMBEDDING_BACKEND}' and hash fallback is not permitted in production. "
            "Set EMBEDDING_BACKEND=bge_m3 or hash_v1."
        )
