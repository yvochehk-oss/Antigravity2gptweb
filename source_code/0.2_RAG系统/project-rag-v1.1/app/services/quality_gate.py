...
def _metadata_match(result: dict, filters: dict) -> bool:
    """Check whether a result matches every active filter field.
    ...
    Returns:
        True if every non-empty filter key matches the result.  A result that
        misses one active constraint is not evidence for that constrained
        query, even if another metadata field happens to match.
    """
    if not filters:
        return False
    checked = False
    for key, want in filters.items():
        ...
        if key == "business_role":
            ...
            wants = [role_aliases.get(str(item).lower(), str(item).lower()) for item in wants]
            got_value = str(got).lower()
        else:
            got_value = str(got)
        if got_value in {str(x) for x in wants}:
            continue
        return False
    return checked
...
def assess_quality(
    ...
    # Hybrid RRF scores are rank scores (typically around 0.01-0.03), while
    # configured rerankers return a normalized [0, 1] relevance score.  When
    # reranking is disabled or unavailable, normalize the hybrid scores by
    # their observed maximum so the gate evaluates relative evidence quality
    # instead of treating every valid lexical/vector hit as near-zero.
    if not any(r.get("rerank_score") is not None for r in reranked_results):
        max_score = max(scores, default=0.0)
        if max_score > 0.0:
            scores = [round(score / max_score, 6) for score in scores]
    top1_score = scores[0]
...
def should_trigger_hyde(gate: QualityGate | dict | None) -> bool:
    ...
    if gate is None:
        return False
    status = gate.get("status") if isinstance(gate, dict) else gate.status
    return status in {"LOW_CONFIDENCE", "INSUFFICIENT_EVIDENCE"}
