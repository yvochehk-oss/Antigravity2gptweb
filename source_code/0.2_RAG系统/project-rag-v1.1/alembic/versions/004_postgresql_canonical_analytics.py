...
revision='004_postgresql_canonical_analytics'; down_revision='003_analytics_project_full'; branch_labels=None; depends_on=None

def _add(table,name,col):
    if name not in {c['name'] for c in sa.inspect(op.get_bind()).get_columns(table)}: op.add_column(table,col)
...
def upgrade():
    ...
    for n,c in project_fields.items(): _add('projects',n,c)
    op.execute("UPDATE projects SET status='ACTIVE' WHERE status IS NULL OR status=''")
    op.alter_column('projects','status',existing_type=sa.String(24),nullable=False,server_default=sa.text("'ACTIVE'"))
    # RAG descriptive columns on the shared internal-entity row.
    fields={
      'entity_type':sa.Column('entity_type',sa.String(32),nullable=True),'industry':sa.Column('industry',sa.String(32),nullable=True),
      'legal_representative':sa.Column('legal_representative',sa.String(80),nullable=True),'legal_rep_id':sa.Column('legal_rep_id',sa.String(32),nullable=True),
      'legal_rep_phone':sa.Column('legal_rep_phone',sa.String(32),nullable=True),'shareholders':sa.Column('shareholders',sa.Text(),nullable=True),
      'supervisor':sa.Column('supervisor',sa.String(80),nullable=True),'finance_officer':sa.Column('finance_officer',sa.String(80),nullable=True),
      'registered_capital':sa.Column('registered_capital',sa.String(40),nullable=True),'establishment_date':sa.Column('establishment_date',sa.String(20),nullable=True),
      'acquisition_date':sa.Column('acquisition_date',sa.String(20),nullable=True),'registration_authority':sa.Column('registration_authority',sa.String(120),nullable=True),
      'registration_number':sa.Column('registration_number',sa.String(40),nullable=True),'unified_social_credit_code':sa.Column('unified_social_credit_code',sa.String(40),nullable=True),
      'business_scope':sa.Column('business_scope',sa.Text(),nullable=True),'contributed_legal':sa.Column('contributed_legal',sa.Numeric(18,2),nullable=True),
      'contributed_shareholder':sa.Column('contributed_shareholder',sa.Numeric(18,2),nullable=True),'note':sa.Column('note',sa.Text(),nullable=True),
      'source':sa.Column('source',sa.String(32),nullable=True),'data_as_of':sa.Column('data_as_of',sa.String(20),nullable=True),
      'created_at':sa.Column('created_at',sa.String(40),nullable=True),'updated_at':sa.Column('updated_at',sa.String(40),nullable=True),
    }
    for n,c in fields.items(): _add('entities',n,c)
    ...
    string_profile_fields = [
      'entity_type','industry','legal_representative','legal_rep_id','legal_rep_phone',
      'shareholders','supervisor','finance_officer','registered_capital','establishment_date',
      'acquisition_date','registration_authority','registration_number',
      'unified_social_credit_code','business_scope','note','source','data_as_of','created_at','updated_at'
    ]
    ...
    op.execute("UPDATE entities SET entity_kind=COALESCE(NULLIF(entity_kind,''),NULLIF(kind,''),'company') WHERE entity_kind IS NULL OR entity_kind=''")
    op.execute("UPDATE entities SET status=CASE WHEN active THEN 'active' ELSE 'inactive' END WHERE status IS NULL OR status=''")
    ...
    # Deterministic views. Ratios are 0..1, not percentage points.
    for v in ['analytics_project_full','analytics_tax','analytics_eac','analytics_cost','analytics_cashflow','analytics_project_profit','analytics_project_summary']:
      op.execute(text(f'DROP VIEW IF EXISTS {v} CASCADE'))
    op.execute(text(r"""CREATE VIEW analytics_project_summary AS
      ...
      FROM projects p"""))
    op.execute(text(r"""CREATE VIEW analytics_project_profit AS
    ...
    op.execute(text(r"""CREATE VIEW analytics_cost AS
      ...
      CASE WHEN b.budget_cost>0 AND a.actual_cost IS NOT NULL THEN (a.actual_cost-b.budget_cost)/b.budget_cost ELSE NULL END cost_variance_rate,
      CASE WHEN a.actual_cost>0 AND b.budget_cost IS NOT NULL THEN b.budget_cost/a.actual_cost ELSE NULL END cpi,CURRENT_TIMESTAMP calculated_at
      FROM analytics_project_summary p LEFT JOIN a ON a.project_id=p.project_id LEFT JOIN b ON b.project_id=p.project_id"""))
    op.execute(text(r"""CREATE VIEW analytics_eac AS
      SELECT p.project_id,p.project_code,p.project_name,p.contract_amount,pp.recognized_revenue,pp.real_project_cost actual_cost,
      CASE WHEN p.contract_amount>0 AND pp.recognized_revenue>0 AND pp.real_project_cost IS NOT NULL
           THEN pp.real_project_cost/(pp.recognized_revenue/p.contract_amount) ELSE NULL END eac_cost,
      p.contract_amount eac_revenue,
      CASE WHEN p.contract_amount>0 AND pp.recognized_revenue>0 AND pp.real_project_cost IS NOT NULL
           THEN p.contract_amount-(pp.real_project_cost/(pp.recognized_revenue/p.contract_amount)) ELSE NULL END eac_profit,
      CASE WHEN p.contract_amount>0 AND pp.recognized_revenue>0 AND pp.real_project_cost IS NOT NULL
           THEN (p.contract_amount-(pp.real_project_cost/(pp.recognized_revenue/p.contract_amount)))/p.contract_amount ELSE NULL END eac_margin,
    ...
    op.execute(text(r"""CREATE VIEW analytics_cashflow AS
    ...
    op.execute(text(r"""CREATE VIEW analytics_tax AS
      WITH iv AS (SELECT project_id,SUM(CASE WHEN direction='out' THEN vat ELSE 0 END) output_vat,
                         SUM(CASE WHEN direction='in' AND deductible THEN vat ELSE 0 END) input_vat FROM invoices GROUP BY project_id)
      ...
      NULL::numeric income_tax_estimate,NULL::numeric surcharge_estimate,NULL::numeric tax_burden_rate,NULL::numeric total_tax_burden,
      CURRENT_TIMESTAMP calculated_at FROM analytics_project_summary p LEFT JOIN iv ON iv.project_id=p.project_id"""))
    op.execute(text(r"""CREATE VIEW analytics_project_full AS
      ...
      t.output_vat,t.input_vat,t.vat_payable,t.tax_burden_rate,
      NULL::numeric health_score,
      (pp.recognized_revenue IS NOT NULL AND pp.real_project_cost IS NOT NULL AND pp.collection_rate IS NOT NULL AND e.eac_profit IS NOT NULL AND e.eac_margin IS NOT NULL) facts_available,
      CURRENT_TIMESTAMP calculated_at
