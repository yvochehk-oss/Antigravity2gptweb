...
def _assert_existing_table_compatible(table_name: str, declarations: tuple[object, ...]) -> None:
    ...
    declared_primary_key_columns = {
        ...
        for column in (
            list(declaration.columns)
            or list(getattr(declaration, "_pending_colargs", ()))
        )
    }
    ...
    for name, column in declared.items():
        ...
        if bool(reflected.get("nullable", True)) != bool(column.nullable):
            raise RuntimeError(
                f"existing table {table_name!r} column {name!r} has incompatible nullability"
            )
        if (name in primary_key_columns) != (
            name in declared_primary_key_columns or bool(column.primary_key)
        ):
            raise RuntimeError(
                f"existing table {table_name!r} column {name!r} has incompatible primary-key state"
            )
    ...
    if expected_unique:
        ...
        if missing:
            raise RuntimeError(
                f"existing table {table_name!r} is missing unique constraints: {sorted(missing)}"
            )

    if expected_checks:
        actual_checks = {
            _sql_signature(check.get("sqltext", ""))
            for check in inspector.get_check_constraints(table_name)
        }
        missing = expected_checks - actual_checks
        if missing:
            raise RuntimeError(
                f"existing table {table_name!r} is missing check constraints: {sorted(missing)}"
            )
    ...
    if expected_foreign_keys:
        ...
        if missing:
            raise RuntimeError(
                f"existing table {table_name!r} is missing foreign keys: {sorted(missing)}"
            )
...
def _assert_existing_index_compatible(index_name: str, table_name: str, columns: list[str], unique: bool) -> bool:
    ...
    for existing in candidates:
        ...
        actual_unique = bool(existing.get("unique", True if existing in inspector.get_unique_constraints(table_name) else False))
...
def upgrade() -> None:
    # Projects
    _create_if_compatible(op.create_table, 'projects',
    ...
    # Entities
    _create_if_compatible(op.create_table, 'entities',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('entity_code', sa.String(16), nullable=True),
    ...
    # Documents
    _create_if_compatible(op.create_table, 'documents',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Chunks
    _create_if_compatible(op.create_table, 'chunks',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Ingest jobs
    _create_if_compatible(op.create_table, 'ingest_jobs',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('document_id', sa.Integer(), nullable=False),
    ...
    # Query logs
    _create_if_compatible(op.create_table, 'query_logs',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('project_id', sa.Integer(), nullable=False),
    ...
    # Benchmark questions
    _create_if_compatible(op.create_table, 'benchmark_questions',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Benchmark runs
    _create_if_compatible(op.create_table, 'benchmark_runs',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('project_id', sa.Integer(), nullable=True),
    ...
    # Backup records
    _create_if_compatible(op.create_table, 'backup_records',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Query feedback
    _create_if_compatible(op.create_table, 'query_feedback',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Knowledge conflicts
    _create_if_compatible(op.create_table, 'knowledge_conflicts',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('project_id', sa.Integer(), nullable=False),
    ...
    # Regulations
    _create_if_compatible(op.create_table, 'regulations',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    # Regulation articles
    _create_if_compatible(op.create_table, 'regulation_articles',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    _create_if_compatible(op.create_index, 'ix_regulation_articles_regulation_id', 'regulation_articles', ['regulation_id'])
    _create_if_compatible(op.create_index, 'ix_regulation_articles_chapter', 'regulation_articles', ['chapter'])

    # Regulation chunks
    _create_if_compatible(op.create_table, 'regulation_chunks',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('regulation_id', sa.Integer(), nullable=False),
        sa.Column('chunk_index', sa.Integer(), nullable=False),
        sa.Column('heading_path', sa.String(500), nullable=False),
    ...
    # === AI Review models ===
    # Facts snapshots
    _create_if_compatible(op.create_table, 'facts_snapshots',
    ...
    # AI review runs
    _create_if_compatible(op.create_table, 'ai_review_runs',
        sa.Column('id', sa.String(36), nullable=False),
        sa.Column('project_id', sa.Integer(), nullable=False),
        sa.Column('project_code', sa.String(64), nullable=False),
    ...
    # Metric versions
    _create_if_compatible(op.create_table, 'metric_versions',
        sa.Column('id', sa.Integer(), nullable=False),
...
def _require_explicit_destructive_downgrade_approval() -> None:
    ...
    if not backup_value:
        raise RuntimeError(
            "populated-database downgrade requires PROJECT_RAG_DOWNGRADE_BACKUP"
        )
    backup = Path(backup_value).expanduser().resolve()
    if not backup.is_file() or backup.stat().st_size == 0:
        raise RuntimeError(
            f"populated-database downgrade backup is missing or empty: {backup}"
        )
