...
# ==============================================================================
# 成都建工 V3.0 · macOS 桌面一键停止器
# 幂等停止：Boss → IDP/RAG/Tax/Ling → 业务端口残留 → 本项目 PostgreSQL
# ==============================================================================
set -u
...
stop_project_postgres() {
  if ! project_postgres_running; then
    print -P "  %F{242}• 本项目 PostgreSQL 未运行或 5432 属于外部实例，跳过。%f"
    return 0
  fi
...
for env_f in "$ROOT_ENV_FILE" "$RUNTIME_ENV_FILE"; do
  if [[ -f "$env_f" ]]; then
    ...
    if ! source "$env_f"; then
      set +a
      print -u2 -- "⚠ 环境文件加载失败：${env_f}，继续执行停止清理。"
    fi
    set +a
  fi
done
...
print -P "%F{220}🗄️  [4/4] 停止本项目 PostgreSQL...%f"
if [[ "${CHENGDU_KEEP_POSTGRES:-0}" == "1" ]]; then
  print -P "  %F{214}• CHENGDU_KEEP_POSTGRES=1，保留 PostgreSQL。%f"
else
...
if [[ "$all_clear" == "true" ]]; then
  print -P "%F{46}==================================================================%f"
  print -P "%F{46}  ✅ 成都建工 V3.0 已清爽停止：业务端口与后台进程已释放%f"
  print -P "%F{46}==================================================================%f"
else
  print -P "%F{220}==================================================================%f"
...
if [[ "${CHENGDU_KEEP_POSTGRES:-0}" == "1" ]]; then
  print -P "  🗄️ PostgreSQL：按配置保留"
elif project_postgres_running; then
  print -P "  🗄️ PostgreSQL：%F{196}仍运行%f"
else
  print -P "  🗄️ PostgreSQL：已停止 / 非本项目实例不处理"
fi
print -- ""
