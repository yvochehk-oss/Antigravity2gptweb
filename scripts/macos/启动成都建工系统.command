...
RAW_SCRIPT_PATH="${(%):-%x}"
[[ -z "$RAW_SCRIPT_PATH" || "$RAW_SCRIPT_PATH" == "zsh" ]] && RAW_SCRIPT_PATH="$0"
...
resolve_script_dir() {
  local source="${1:-${RAW_SCRIPT_PATH}}" base target
  while [[ -L "$source" ]]; do
...
resolve_project_dir() {
  ...
  if [[ -n "${CHENGDU_PROJECT_DIR:-}" ]]; then
    ...
    [[ -f "${dir}/start_all.sh" && -d "${dir}/source_code" ]] || return 1
    print -r -- "$dir"
    return 0
  fi
  dir="$SCRIPT_DIR"
  for i in 1 2 3 4 5 6; do
    if [[ -f "${dir}/start_all.sh" && -d "${dir}/source_code" ]]; then
      print -r -- "$dir"
      return 0
    fi
    parent="$(dirname -- "$dir")"
  ...
  local fallback="/Users/yvoche/AI开发/073_成都建工/V3.0"
  if [[ -f "${fallback}/start_all.sh" && -d "${fallback}/source_code" ]]; then
    print -r -- "$fallback"
    return 0
  fi
  return 1
...
SCRIPT_DIR="$(resolve_script_dir "$RAW_SCRIPT_PATH")" || { print -u2 -- "❌ 无法解析启动脚本所在目录。"; exit 1; }
