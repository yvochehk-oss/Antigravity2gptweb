...
rem ============================================================
rem ?¡¤ SSOT:
rem   - ??data dirpostmaster pid ? PostgreSqlPortNegotiator §Õ
...
if "%PORT%"=="0" (
    rem ?§¿? C# ????
    echo [PostgreSQL] ¦Ä? runtime\state\postgres.json?? PostgreSqlPortNegotiator §¿?
    exit /b 0
)
...
if not defined PG_CTL (
    echo [PostgreSQL] : ¦Ä?§Á pg_ctl.exe???§Ø??
    exit /b 2
)
...
call :IS_READY
if not errorlevel 1 (
    echo [PostgreSQL] §Á PostgreSQL  127.0.0.1:%PORT% 
    exit /b 0
)
...
if exist "!DATA_DIR!\postmaster.pid" (
    ...
    if not errorlevel 1 (
        echo [PostgreSQL] : ??? PostgreSQL ?? %PORT% ¦Ä
...
for /l %%K in (1,1,30) do (
    call :IS_READY
    if not errorlevel 1 (
        echo [PostgreSQL] §Á? 127.0.0.1:%PORT% 
        exit /b 0
    )
    ping 127.0.0.1 -n 2 >nul
)

echo [PostgreSQL] :  30 ¦Ä %PORT% ?§Ñ??
"!PG_CTL!" status -D "!DATA_DIR!" >nul 2>&1
