...
netstat -ano | findstr "LISTENING" | findstr ":54320 :5432" >nul
if errorlevel 1 (
...
:: 2. LLM (8930)
netstat -ano | findstr "LISTENING" | findstr ":8930" >nul
if errorlevel 1 (
    ...
    if not exist "!MODEL_FILE!" (
        ...
        set "MODEL_ALIAS=ling-3.0-tiny"
    )
    if exist "!SERVER_BIN!" (
        start "01_LLM" /B "!SERVER_BIN!" --model "!MODEL_FILE!" --host 127.0.0.1 --port 8930 --alias "!MODEL_ALIAS!" --ctx-size 16384 --threads 4 --threads-batch 4 --batch-size 512 --ubatch-size 256 --gpu-layers 0 --reasoning off --parallel 1 --jinja > "logs\llm.log" 2>&1
    )
)
