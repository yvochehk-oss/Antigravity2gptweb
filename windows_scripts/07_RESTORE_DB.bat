@echo off
@chcp 936 >nul 2>&1
title 成都建工 V3.0 - PostgreSQL 数据库一键导入 (Windows)
...
echo ==============================================================================
echo   成都建工 V3.0 - 数据库一键还原工具 (PostgreSQL)
echo ==============================================================================
echo.
...
if not exist "%DUMP_FILE%" set "DUMP_FILE=projectrag_backup_full_20260827.dump"
...
pg_restore -U %PG_USER% -p %PG_PORT% -d %PG_DB% --no-owner --clean --if-exists "%DUMP_FILE%" 2>tmp_pg_restore_err.txt
set "RESTORE_CODE=!errorlevel!"
if !RESTORE_CODE! NEQ 0 (
    findstr /i "unsupported version" tmp_pg_restore_err.txt >nul 2>&1
    if not errorlevel 1 (
        echo [警告] pg_restore 版本不兼容 - dump 由高版本PG生成！
        echo [自动降级] 切换至 SQL 文本备份还原模式...
        set "SQL_FILE=database\projectrag_latest.sql"
        if not exist "!SQL_FILE!" set "SQL_FILE=projectrag_latest.sql"
        if exist "!SQL_FILE!" (
            echo [执行] psql -f !SQL_FILE! ...
            psql -U %PG_USER% -p %PG_PORT% -d %PG_DB% -v ON_ERROR_STOP=0 -f "!SQL_FILE!" 2>nul
            echo [完成] SQL 文本备份还原执行完毕。
        ) else (
            echo [错误] 未找到 database\projectrag_latest.sql，请检查部署包完整性！
